  ...
  try {
    ...
    if (agentName === "Lead Discovery") {
      await updateRun(
        "running",
        20,
        "Scanning Southern California directories and Maps..."
      );
      await sleep(1500);
      ...
      await updateRun(
        "success",
        100,
        `Discovered ${newLeads.length} new qualified bridge lender prospects.`
      );
      return;
    }
    ...
    if (agentName === "Scoring") {
      ...
      await updateRun(
        "running",
        40,
        "Analyzing digital footprint, SEO visibility, and GBP presence..."
      );
      await sleep(1500);
      ...
      await updateRun(
        "success",
        100,
        "Lead scored. HIGH priority — strong digital visibility gaps detected."
      );
      return;
    }
    ...
    if (agentName === "Asset Generation") {
      ...
      await updateRun(
        "running",
        30,
        "Building AI SEO audit report and custom preview website..."
      );
      await sleep(2000);
      ...
      await blink.db.generatedAssets.create({
        ...
        content:
          "Comprehensive SEO, AEO, and GEO audit revealing 14 critical visibility gaps.",
      });
      ...
      await blink.db.generatedAssets.create({
        ...
        content:
          "AEO-optimized custom preview website — fully branded and ready to deploy.",
      });
      ...
      await updateRun(
        "success",
        100,
        "Assets generated: 1 audit report + 1 custom website preview."
      );
      return;
    }
    ...
    if (agentName === "Outreach") {
      ...
      await updateRun(
        "running",
        50,
        "Composing value-first outreach email with audit + website preview..."
      );
      await sleep(1500);
      ...
      await updateRun(
        "success",
        100,
        "Step 1 sent: Value-First email with audit + preview. Monitoring for 48h response window."
      );
      return;
    }
    ...
    if (agentName === "Invoicing") {
      ...
      await updateRun(
        "running",
        60,
        "Generating Growth Package invoice ($4,997)..."
      );
      await sleep(1000);
      ...
      await updateRun(
        "success",
        100,
        "Stripe invoice for $4,997 (Growth Package) generated and sent to lead."
      );
      return;
    }
    ...
    if (agentName === "Repurposing") {
      ...
      await updateRun(
        "running",
        80,
        "48h deadline passed. Repurposing assets for next high-score prospect..."
      );
      await sleep(1000);
      ...
      await updateRun(
        "success",
        100,
        "Assets re-branded and queued for next qualified prospect. Revenue protected."
      );
      return;
    }
    ...
    if (agentName === "Coordinator" || agentName === "Full Pipeline") {
      await updateRun(
        "running",
        5,
        "Initializing autonomous acquisition cycle for SoCal bridge lenders..."
      );
      await sleep(500);
      ...
      await updateRun(
        "running",
        12,
        "Stage 1: Scanning Google Maps, Yelp, and commercial lender directories..."
      );
      await sleep(1500);
      ...
      await updateRun(
        "running",
        28,
        `Discovered ${newLeads.length} new bridge lender prospects.`
      );
      await sleep(800);
      ...
      await updateRun(
        "running",
        42,
        "Stage 2: Running AI SEO, AEO, and GBP visibility audits..."
      );
      await sleep(1500);
      ...
      await updateRun(
        "running",
        65,
        "Stage 3: Deploying value-first outreach sequences with audit + website preview..."
      );
      await sleep(1500);
      ...
      await updateRun(
        "running",
        82,
        "Stage 4: Monitoring responses... Inbound 'yes' detected from top prospect!"
      );
      await sleep(1200);
